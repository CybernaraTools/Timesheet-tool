...
import { useRequestOtp, useVerifyOtp, useCompleteInvite } from '@/lib/hooks/useAuth';
...
export default function InvitePage() {
  ...
  const handleStep1 = (data) => {
    requestOtp.mutate({ email: data.email, purpose: 'invite' }, {
      onSuccess: () => {
        setEmail(data.email);
  ...
  const handleStep2 = (data) => {
    verifyOtp.mutate({ email, code: data.code, purpose: 'invite' }, {
      onSuccess: () => {
        setOtp(data.code);
  ...
  const handleStep3 = (data) => {
    if (!inviteToken) {
      toast.error("Invitation token is missing in URL");
      return;
    }
    ...
    completeInvite.mutate({
      ...
      onSuccess: (res) => {
        ...
        if (token && res?.user) {
          ...
          toast.success("Account activated successfully! Welcome to Cybernara.");
  ...
  const isLoading = requestOtp.isPending || verifyOtp.isPending || completeInvite.isPending;
